...
def transform_data(trdata_x, trdata_y, valdata_x, valdata_y, info_idx, info_expl, params):
    ...
    tf_val_x, _ = adjust_outliers(valdata_x, info_expl['df_lim'], idx_rm_col)
    ...
    nm_floats = [nm for nm in col_names if len(tf_train_x[nm].value_counts()) > max_distinct_n]
    cc_miss = graph_info['mi_c'].where(graph_info['mi_c'] > max_cc_pct).dropna().index
...
for nr_file in file_range:
    ...
    for this_fold in fold_range:
        ...
        # preprocessing
        tf_train_x, tf_val_x, info_tf = transform_data(train_x,val_x,idx)

        # print fold 0 to check
        if this_fold == 0:
            tf_train_x.to_csv('../preprocessing/' + data_name + 'tf_tr_' + str(this_fold) + '.csv')
            tf_val_x.to_csv('../preprocessing/' + data_name + 'tf_val_' + str(this_fold) + '.csv')
        ...
        modes = tf_train_x.mode(axis=0)
        cc = tf_train_x.eq(modes.loc[0,]).sum() / tf_train_x.shape[0]
        idx_painfully_concentrated = cc.where(cc> 0.99).dropna().index
        tf_train_x.drop(idx_painfully_concentrated,axis=1)